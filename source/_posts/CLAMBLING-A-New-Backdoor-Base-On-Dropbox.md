...
tags:
    ...
    - IncidentResponse
---
2019 年 7 月，我們發現一個合作的客戶疑似遭受 APT 攻擊並立刻著手調查。調查過程中發現了一種全新的後門樣本，該樣本的特殊之處在於攻擊者 Dropbox API 實現了一個具備多種功能的後門惡意程式，並且完美地將 C&C 伺服器建構在 Dropbox 上。透過惡意程式分析，我們獲得了樣本所使用的 Dropbox API Token 並且能夠進一步的深入探討整個架構的運作原理。<!--more-->
...
![圖 1. 舊版 mpsvc.dll](clambling-01.png)
...
![圖 2. 新版 mpsvc.dll](clambling-02.png)
...
![圖 3. mpsvc.dll 裡解碼後的 shellcode](clambling-03.png)
![圖 4. 最終在記憶體裡的惡意程式執行檔](clambling-04.png)
...
![圖 5. 繞過 UAC 的部分程式碼](clambling-05.png)
...
![圖 6. 註冊成為開機自動執行程式](clambling-06.png)
若具備 Administrator 權限，則會將自己註冊為系統 Service (`HKEY_LOCAL_MACHINE\\SYSTEM\\CurrentControlSet\\services`)。

![圖 7. 註冊成為系統服務](clambling-07.png)
### 資訊收集

![圖 8. 資訊收集的部分程式碼](clambling-08.png)
...
![圖 9. 進行側錄資料編碼的部分程式碼](clambling-09.png)
...
![圖 10. 準備假的 HTTP POST 請求的部分程式碼](clambling-10.png)
...
![圖 11. Dropbox API token 字串陣列的部分程式碼](clambling-11.png)
...
![圖 12. 與 Dropbox 互動的部分程式碼](clambling-12.png)
...
![圖 13. 整理過後的受感染電腦清單](clambling-13.png)
...
![圖 14. 三個特殊檔案與 Dropbox 互動的流程圖](clambling-14.png)
...
![圖 15. 受感染電腦的命令執行結果清單](clambling-15.png)
![圖 16. 另一台受感染電腦的命令執行結果清單](clambling-16.png)
...
![圖 17. 攻擊者大略的作息時間](clambling-17.png)

## 總結

在分析並取得 Dropbox Token 之後，我們持續監控各 Dropbox APP 內的檔案變化，可以觀察出在 2019 年 7 月 ~ 2019 年 9 月兩個月之中受感染電腦的數量變化。

![圖 18. Dropbox A 的感染電腦數量變化圖](clambling-18.png)
![圖 19. Dropbox B 的感染電腦數量變化圖](clambling-19.png)
